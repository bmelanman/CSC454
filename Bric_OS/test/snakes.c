...
static char colors[] = { VGA_BLACK, VGA_BLUE,   VGA_RED,   VGA_GREEN, VGA_MAGENTA,
                         VGA_CYAN,  VGA_YELLOW, VGA_WHITE, VGA_BLACK };
...
static int endsnake = 0;  // a flag, that when set causes the currently running snake to exit
...
static void srand( int newseed ) { seed = (unsigned)newseed & 0x7fffffffffffffffU; }

int rand( void )
{
    seed = ( seed * 6364136223846793005U + 1442695040888963407U ) & 0x7fffffffffffffffU;
    return (int)seed;
}
...
snake new_snake( int y, int x, int len, int dir, int color )
{
    ...
    snake new = (snake)kmalloc( sizeof( struct snake_st ) );
    int i;
...
{
    // deallocate the given snake. First remove it from allsnakes, then free its bits
    snake sp;
...
void draw_food()
{
    VGA_display_attr_char_pos( food.x, food.y, SN_FOOD_CHAR, VGA_WHITE, VGA_BLACK );
}
...
{
    snake *s = (snake *)arg;
    // run a snake until it eats enough food.  We keep track of how much each snake has eaten by its
    // color.

    // Snakes get brighter and brighter until they turn white then wink out.
    ...
    while ( true )
    {
        ...
        // if the new head is on the food, count it as eaten and replace the food
        if ( onfood( *s ) )
        {
...
static int obstructed( sn_point p )
{
    ...
    if ( p.x >= cols || p.x < 0 || p.y >= rows || p.y < 0 )
    {
        ...
        for ( s = allsnakes; s && !ret; s = s->others )
        {  // look for snakes
            for ( i = 0; i < s->len; i++ )
...
static void move_snake( snake s )
{
    ...
    while ( obstructed( newhead ) && !alldead )
    {
        deadends[s->dir] = 1;
        s->dir = rand() % NUMDIRS;
        if ( s->dir >= NUMDIRS ) s->dir = NUMDIRS - 1;
...
{
    // return the direction of the food (well, as close as this system can get).
    int deltaX, deltaY;
...
static void draw_snake( snake s )
{
    ...
    VGA_display_attr_char_pos(
        s->body[0].x, s->body[0].y, head[s->dir], colors[s->color], VGA_BLACK
    );

    for ( i = 1; i < s->len; i++ )
    {
        VGA_display_attr_char_pos(
            s->body[i].x, s->body[i].y, SN_BODY_CHAR, colors[s->color], VGA_BLACK
        );
    }
}
...
static void erase_snake( snake s )
{
    ...
    for ( i = 0; i < s->len; i++ )
    {
        VGA_display_attr_char_pos( s->body[i].x, s->body[i].y, ' ', VGA_BLACK, VGA_BLACK );
    }
}

void kill_snake()
{
    // set the flag so that the next time run_snake goes around it's killed
    endsnake = 1;
}

static uint snake_delay = 10;  // default 50 msec
extern uint get_snake_delay()
{
...
{
    // set the snake delay between moves to the given number of milliseconds
    snake_delay = msec;
}
...
{
    // return a pointer to the snake with the given lw_pid, or NULL if it doesn't exist
    (void)lw_pid;
